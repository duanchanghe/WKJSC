 ...
 *  appKey       第三方平台的appKey（QQ平台为appID）
 *  appSecret    第三方平台的appSecret（QQ平台为appKey）
 ...
 *
 *   platformType @see UMSocialPlatformType
 *  shareObmessageObjectrrentViewController 用于弹出类似邮件分享、短信分享等这样的系统页面
 *   completion   回调
 *   currentViewController 只正对sms,email等平台需要传入viewcontroller的平台，其他不需要的平台可以传入nil
 */
 ...
 *
 *  userDefinePlatformProvider 用户自定义的userDefinePlatformProvider必须实现UMSocialPlatformProvider
 *  platformType               @see platformType platformType的有效范围在 （UMSocialPlatformType_UserDefine_Begin,UMSocialPlatformType_UserDefine_End)之间
 *
